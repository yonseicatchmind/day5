		            ...
		            try {
		                ...
		                if (message.contains(":")) {
							...
							if (pars[0].equals("Pencil")) {
		                	   ...
		                	   Platform.runLater(() -> {
		                		   if(message.contains("/q")) { //문제 출제시>>>>> /q제시어 이런 형식으로 채팅을 쳐야함
		                			   TalkBoard.appendText("["+message.substring((message.indexOf("[")+1), message.indexOf("]")) + "] 가 문제를 출제하였습니다.\n");
		                			   ans=message.substring(message.lastIndexOf("q")+1, message.lastIndexOf("q")+4); //제시어 저장
		                		   }
		                		   else
		                			   TalkBoard.appendText(message); //제시어 출제가 아니면 그냥 출력
		                		   if(ans.equals(message.substring(message.indexOf("]")+2,message.length()))) { //제시어랑 채팅이랑 맞으면
